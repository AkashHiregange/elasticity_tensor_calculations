...
import pickle
import numpy as np
from pymatgen.analysis.elasticity import strain, Strain, Deformation, elastic, diff_fit
...
try:
    f = open('elasticity_tensor_rSCAN.txt', 'x')
    f.close()
except:
    print('error')
...
f = open('elasticity_tensor_rSCAN.txt', 'w')
...
f.write('Elasticity tensor in full. The above values were the unique values in the tensor\n')
f.write(str(np.array(elasticity_tensor[0])) + '\n')
f.write('Stress tensor for all deformations in full. Shape Nx3x3 where N is the number of deformations used to compute'
        'the elasticity tensor using the stress strain relation ship \n')
f.write(str(stress_tensor)+'\n')
f.write(str(stress_tensor.shape)+'\n')
f.write('Strain tensor for all deformations in full. Shape Nx3x3 where N is the number of deformations used to compute'
        'the elasticity tensor using the stress strain relation ship \n')
